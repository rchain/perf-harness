...
object Deploy {
  ...
  def deploy(
    ...
    val (cn, contract): (String, String) = session("contract")
      .as[(String, String)]
    val x1 = System.currentTimeMillis()
    println(s"starting deploy of $cn on client ${client.full} session ${session.userId}")
    val d = DeployData()
...
object GetDataFromBlock {
  def getData(
      dataName: String
  )(session: Session)(client: ClientWithDetails): Future[(Session, DeployServiceResponse)] = {
    ...
    println(s"getting binary data of $cn on client ${client.full} session ${session.userId}")
    ...
    parData.map { either =>
      ...
      res match {
        ...
        case _ =>
          println(s"$res")
          (session.set("client", client), DeployServiceResponse("Failed to get binary data"))
      }
    }
  }
}
...
class RNodeRequestAction(
    ...
    val request: Session => ClientWithDetails => Future[(Session, DeployServiceResponse)],
  ...
  def logResponse(
    ...
    statsEngine.logResponse(ns, logName, timings, ok, None, Some(msg), List(logName))
    ns
  }
...
abstract class RNodeActionBuilder extends ActionBuilder {
  val execute: Session => ClientWithDetails => Future[(Session, DeployServiceResponse)]
  val actionName: String
...
object RNodeProtocol {
  ...
  def createFor(hostStrings: List[String]): RNodeProtocol = {
    val mapped = hostStrings.map { host =>
      val s = host.split(":")
      assert(s.size == 2, s"Invalid host string $s, expected format is address:port")
      val address = s(0)
      val port    = s(1).toInt
      (address, port)
  ...
  val RNodeProtocolKey = new ProtocolKey {
    ...
    ): RNodeProtocol => RNodeComponents = { rnodeProtocol =>
      {
...
case class RNodeComponents(
    rnodeProtocol: RNodeProtocol,
